...
import {schemaRememberedLoginWithPassword} from '~/Common/validations/auth/login';
...
export function RememberedIndividual() {
  ...
  type FormType = yup.InferType<typeof schemaRememberedLoginWithPassword>;

  const {handleSubmit, control, formState, setValue} = useForm<FormType>({
    mode: 'onChange',
    resolver: yupResolver(schemaRememberedLoginWithPassword),
    defaultValues: {
          ...
          AuthActions.createOtp()
            .then(otpResponse => {
              GoToOtp({
                ...
                ...otpResponse.data,
                ...response.data,
                isMainCustomerDocumentConfirmed: false,
                isMainCustomerShowedApprovementScreen: false,
              });
            })
            .catch(handleError)
            .finally(hideHud);
    ...
    <View style={[layoutStyles.fullscreenContainer, layoutStyles.spaceBetween]}>
      <KeyboardAwareScrollView
        ...
        <View style={[layoutStyles.alignCenter, layoutStyles.justifyCenter]}>
          ...
          <AvatarWithName
            name={[
              CustomerDataProvider.Data.name,
              CustomerDataProvider.Data.surname,
            ].join(' ')}
